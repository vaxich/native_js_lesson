...
//console.log(user === copyUser);// false - разные объекты , не могут быть равны
//console.log(user.friends === copyUser.friends);// true - объекты разные, но так как это копия, то НЕ ПРИМИТИВЫ РАВНЫ, т.к. это одна ссылка

//2. Полная (глубокая) копия student
//const deepCopyStudent = { ...user, friends: [...user.friends] };
//console.log(user === deepCopyStudent);// false false - разные объекты , не могут быть равны
//console.log(user.friends === deepCopyStudent.friends); // false - т.к копия глубокая, в ней вложенные объекты не равны

//3. Поверхностная копия students
//const copyStudents = [...students];
//slice()
//console.log(students === copyStudents);//false - копируем объекты - они не могут быть равны
//console.log(students[0] === copyStudents[0]);// true - сравниваем ссылки на объекты

//4*. Полная (глубокая) копия students
//const deepCopyStudents = students.map((st) => ({ ...st }));
//console.log(students === deepCopyStudents);// false - копируем объекты - они не могут быть равны
//console.log(students[0] === deepCopyStudents[0]);//false - копируем объекты - они не могут быть равны, т.к. копирование глубокое
...
//console.log(deepCopyStudents.sort((a, b) => b.scores - a.scores));// не понял
//5a. Отсортируйте студентов по алфавиту
// function sortByName(a, b) {// тоже не понял
...
//const bestStudents = students.filter((st) => st.scores >= 100);//фильтрация
//console.log(bestStudents);

//6a.Сформируйте массив из трёх лучших студентов
//const topStudents = deepCopyStudents.splice(0, 3);// не понял
//console.log(topStudents);
//console.log(deepCopyStudents);

//6b. Объедините массивы deepCopyStudents и topStudents так,
// чтоб сохранился порядок сортировки
 //const newDeepCopyStudents = [...topStudents, ...deepCopyStudents];//копируем в нужном порядке
 //console.log(newDeepCopyStudents);

//7. Сформируйте массив холостых студентов
//const notMarriedStudents = students.filter((st) => !st.isMarried);//фильтр
//console.log(notMarriedStudents);

//8. Сформируйте массив имён студентов
//const studentsNames = students.map((st) => st.name);// мапим и берем только имена
...
//const trueStudents = students.map((st) => ({ ...st, isStudent: true }));// мипим и добавляем свойство
//console.log(trueStudents);

//10. Nick женился. Выполните преобразование массива students
// let studentsWithMarriedNick = students.map(st => st.name === "Nick" ? {...st, isMarried: true} : st);//мапим, находим имя, и меняем свойство
...
//const ann = students.find((st) => st.name === "Ann");// через find находим и записываем в новую переменную
//console.log(ann);
//console.log(students);

//12. Найдите студента с самым высоким баллом
//  const bestStudent = students.reduce((acc, st) => {// вообще не понял
